  ...
  if (object && source) {
    ...
    for (key in source) {
      if (proto[key] === undefined) {
        // Check for getter and setters
        var g = source.__lookupGetter__(key), s = source.__lookupSetter__(key);
        if ( g || s ) {
            if ( g && object.__lookupGetter__(key) === undefined )
                object.__defineGetter__(key, g);
            if ( s && object.__lookupSetter__(key) === undefined )
                object.__defineSetter__(key, s);
            continue;
        }
        var value = source[key];
        // Object detection.
